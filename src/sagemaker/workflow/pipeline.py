...
def _validate_role_arn(role_arn, parameter_name="role_arn"):
    """Validate that role_arn is a string if not None.

    Args:
        role_arn: The role ARN value to validate.
        parameter_name (str): The name of the parameter for error messaging.
    ...
    if role_arn is not None and not isinstance(role_arn, str):
        raise ValueError(
            "Expected a string value for parameter '{}', but received a value of type '{}': {}".format(
                parameter_name, type(role_arn).__name__, role_arn
            )
        )
