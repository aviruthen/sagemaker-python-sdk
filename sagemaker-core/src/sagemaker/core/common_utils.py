...
def _create_or_update_code_dir(
    ...
    for dependency in dependencies:
        ...
        if not dep_basename or dep_basename in (".", ".."):
            raise ValueError(f"Invalid dependency path: {dependency}")
        if os.path.isdir(dependency):
            shutil.copytree(dependency, os.path.join(lib_dir, dep_basename))
        else:
...
def _extract_model(model_uri, sagemaker_session, tmp):
    ...
    if model_uri.lower().startswith("s3://"):
        ...
        # Validate local model path does not access sensitive system paths
        abs_model_path = abspath(realpath(local_model_path))
        for sensitive_path in _SENSITIVE_SYSTEM_PATHS:
            if abs_model_path != "/" and abs_model_path.startswith(sensitive_path):
                raise ValueError(
                    f"model_uri cannot access sensitive system paths. "
                    f"Got: {model_uri} (resolved to {abs_model_path})"
                )
...
def resolve_nested_dict_value_from_config(
    ...
    if config_value is None and default_value is None:
        # if there is nothing to set, return early. And there is no need t

... [FILE TRUNCATED: This file is 85116 characters (1063 lines) which exceeds the 50000 character limit. Only the first 50000 characters are shown. Consider using search_code to find specific functions or sections instead of reading the entire file.]
