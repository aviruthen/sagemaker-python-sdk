...
def test_pipeline_create_with_non_string_role_arn(sagemaker_session_mock):
    mock_session = copy.deepcopy(sagemaker_session_mock)
    mock_session.sagemaker_config = {}
    pipeline = Pipeline(
        name="MyPipeline",
    ...
    with pytest.raises(ValueError, match="Expected a string value for parameter 'role_arn'"):
        pipeline.create(role_arn={"arn": "arn:aws:iam::111111111111:role/FooRole"})
    with pytest.raises(ValueError, match="Expected a string value for parameter 'role_arn'"):
        pipeline.update(role_arn={"arn": "arn:aws:iam::111111111111:role/FooRole"})
    with pytest.raises(ValueError, match="Expected a string value for parameter 'role_arn'"):
        pipeline.upsert(role_arn={"arn": "arn:aws:iam::111111111111:role/FooRole"})
    with pytest.raises(ValueError, match="Expected a string value for parameter 'role_arn'"):
        pipeline.create(role_arn=12345)
    with pytest.raises(ValueError, match="Expected a string value for parameter 'role_arn'"):
        pipeline.update(role_arn=12345)
    with pytest.raises(ValueError, match="Expected a string value for parameter 'role_arn'"):
        pipeline.upsert(role_arn=12345)
    with pytest.raises(ValueError, match="Expected a string value for parameter 'role_arn'"):
        pipeline.create(role_arn=["arn:aws:iam::111111111111:role/FooRole"])


def test_pipeline_put_triggers_with_non_string_role_arn(sagemaker_session_mock):
    mock_session = copy.deepcopy(sagemaker_session_mock)
    mock_session.sagemaker_config = {}
    pipeline = Pipeline(
        name="MyPipeline",
    ...
    with pytest.raises(ValueError, match="Expected a string value for parameter 'role_arn'"):
        pipeline.put_triggers(triggers=[Mock()], role_arn={"arn": "arn:aws:iam::111111111111:role/FooRole"})
    with pytest.raises(ValueError, match="Expected a string value for parameter 'role_arn'"):
        pipeline.put_triggers(triggers=[Mock()], role_arn=12345)
