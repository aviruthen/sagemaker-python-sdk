...
def repack(inference_script, model_archive, source_dir=None):  # pragma: no cover
    """Repack custom code into an existing model TAR archive
    ...
    # With ModelTrainer, the input data is available in the training channel
    data_directory = "/opt/ml/input/data/training"
    archive_basename = os.path.basename(model_archive)
    # Validate the basename to prevent path traversal
    if not archive_basename or archive_basename in (".", "..") or os.sep in archive_basename:
        raise ValueError(f"Invalid model archive name: {model_archive}")
    model_path = os.path.join(data_directory, archive_basename)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Validate inference_script does not contain path traversal components
        if os.sep in inference_script or inference_script.startswith(".."):
            raise ValueError(f"Invalid inference script path: {inference_script}")

        if source_dir:
            # copy /opt/ml/input/data/code to code/ (ModelTrainer structure)
            source_code_path = "/opt/ml/input/data/code"
            if os.path.exists(source_code_path):
                if os.path.exists(code_dir):
                    shutil.rmtree(code_dir)
                shutil.copytree(source_code_path, code_dir)
            else:
                # Fallback to /opt/ml/code for backward compatibility
                if os.path.exists(code_dir):
                    shutil.rmtree(code_dir)
            ...
            for path in entry_point_paths:
                if os.path.exists(path):
                    ...
                    break
            if entry_point:
                dest_path = os.path.join(code_dir, inference_script)
                # Verify destination is within code_dir
                resolved_dest = _get_resolved_path(dest_path)
                resolved_code = _get_resolved_path(code_dir)
                if not resolved_dest.startswith(resolved_code + os.sep) and resolved_dest != resolved_code:
                    raise ValueError(
                        f"Inference script destination {dest_path} escapes code directory"
                    )
                shutil.copy2(entry_point, dest_path)

        # Note: Requirements.txt dependencies are automatically installed by ModelTrainer
        # before this script runs, so no additional installation is needed here.

        # Verify the code directory exists and is within src_dir before copying
        resolved_src = _get_resolved_path(src_dir)
        resolved_code = _get_resolved_path(code_dir)
        if not resolved_code.startswith(resolved_src + os.sep):
            raise ValueError("Code directory is not within the source directory")
